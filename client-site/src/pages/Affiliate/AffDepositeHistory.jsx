import ReasonModal from "@/components/betjili/shared/Modals/ReasonModal";
...
const AffDepositeHistory = () => {
  ...
  const myDeposits = allDeposits?.filter(
    (deposit) => deposit.parentId === user?._id
  );
  // Pagination state
  ...
  const handleStatusClick = async (deposit, selectedStatus) => {
    if (selectedStatus === "rejected") {
      setSelectedDeposit(deposit);
      setStatus("rejected");
      setModalOpen(true);
    } else {
      if (singleUser?.balance < selectedDeposit?.amount) {
        addToast("Insufficient balance to accept this deposit", {
          appearance: "error",
          autoDismiss: true,
        });
        return;
  ...
  const sortedDeposits = [...(myDeposits || [])].sort((a, b) => {
          ...
          <tbody>
            {currentItems?.map((deposit) => (
              ...
              >
                ...
                <td className="p-3 border-r border-r-blue-300 text-left">
                  {deposit.userInputs &&
                  Object.keys(deposit.userInputs).length > 0 ? (
...
export default AffDepositeHistory;
